...
from .dataset import PinaDatasetFactory, PinaTensorDataset, PinaGraphDataset
...
class Collator:
    ...
    @staticmethod
    def _collate_graph_dataset(data_list):
        if isinstance(data_list[0], LabelTensor):
...
class PinaDataModule(LightningDataModule):
    ...
    def _create_dataloader(self, split, dataset):
        ...
        if self.batch_size is not None:
            ...
            is_tensor_dataset = True if isinstance(dataset,
                                                PinaTensorDataset) else False
            if self.automatic_batching:
                
                collate = Collator(self.find_max_conditions_lengths(split),
                                   is_tensor_dataset=is_tensor_dataset)
            else:
                collate = Collator(None, dataset=dataset, is_tensor_dataset=is_tensor_dataset)
